...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className=" uppercase text-xl md:text-3xl mt-[5px] transition-color duration-150 text-black">
          Oktech
        </h2>
      </Link>
      ...
      >
        <ul className="flex flex-col justify-center items-end gap-8 text-xl font-bold text-black p-8">
          <li>
            <Link className="z-20" onClick={handleLinkClick} href="/">
              Strona Główna
            </Link>
          </li>
          <li>
      ...
      <div className="hidden lg:block">
        <BtnLink link="/kontakt">
          <FaPhone /> Zadzwoń już teraz
        </BtnLink>
      </div>

      <div
        className={` ${onTop ? "hidden" : "absolute"} bottom-0 left-0 bg-gradient-to-r from-transparent via-neutral-400 to-transparent w-full h-[1px]`}
      ></div>
    </motion.nav>
  );
};
